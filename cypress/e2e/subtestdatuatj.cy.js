...
it('should highlight even numbers and leave odd numbers unhighlighted', () => {
  ...
  cy.origin('https://login.inbox.lv', () => {
    cy.contains('Slepenais jautājums').parentsUntil('body').contains('Rediģēt').click();
    
    cy.get('.btn.btn-primary.email-recovery-add').click();
    cy.wait(1000);
    cy.get('#pass_recovery_email_form_sec_email', { timeout: 10000 }).should('be.visible').type('vandaanda245@gmail.com');
    cy.get('.btn.btn-primary.float-end').should('be.visible').click();
  });
